...
---
# <a name="design-details-central-concepts-of-the-planning-system"></a><a name="design-details-central-concepts-of-the-planning-system"></a><a name="design-details-central-concepts-of-the-planning-system"></a>Designdetails: Zentrale Konzepte des Planungssystems

Die Planungsfunktionen sind in einer Stapelverarbeitung enthalten, die zuerst die entsprechenden Artikel und die Periode für die Planung auswählt. Dann ruft der Batch-Job gemäss dem Low-Level-Code jedes Artikels (Stücklistenposition) eine Codeeinheit auf, die einen Lieferplan berechnet. Die Codeeinheit gleicht Angebot-Nachfrage-Sätze aus und schlägt dem Benutzer Massnahmen vor, die er ergreifen soll. Die vorgeschlagenen Aktionen erscheinen als Zeilen im Planungs‑ oder Bestellarbeitsblatt.  
...
## <a name="planning-parameters"></a><a name="planning-parameters"></a><a name="planning-parameters"></a>Planungsparameter
...
## <a name="planning-starting-date"></a><a name="planning-starting-date"></a><a name="planning-starting-date"></a>Startdatum der Planung
...
* Alle Vorräte und der Bedarf vor dem Startdatum des Planungszeitraums werden als Teil des Bestands oder als ausgeliefert betrachtet. Mit anderen Worten: Es nimmt an, dass der Plan für die Vergangenheit gemäss dem vorhandenen Plan ausgeführt wurde. Weitere Informationen finden Sie unter [Aufträge vor dem Planungsstartdatum bearbeiten](design-details-balancing-demand-and-supply.md#process-orders-before-the-planning-start-date).  

## <a name="dynamic-order-tracking-pegging"></a><a name="dynamic-order-tracking-pegging"></a><a name="dynamic-order-tracking-pegging"></a>Dynamisches Auftragstracking (Bedarfsverursacher)
...
:::image type="content" source="media/nav_app_supply_planning_1_dynamic_order_tracking.png" alt-text="Beispiel für die dynamische AuftragsTracking.":::
...
### <a name="dynamic-order-tracking-versus-the-planning-system"></a><a name="dynamic-order-tracking-versus-the-planning-system"></a><a name="dynamic-order-tracking-versus-the-planning-system"></a>Dynamisches Auftragstracking gegenüber dem Planungssystem
...
:::image type="content" source="media/nav_app_supply_planning_1_dynamic_order_tracking_graph.png" alt-text="Beispiel für die AuftragsTracking in der Lieferplanung.":::
...
:::image type="content" source="media/nav_app_supply_planning_1_planning_graph.png" alt-text="Beispiel für die AuftragsTracking in der Lieferplanung 2.":::  
...
## <a name="sequence-and-priority-in-planning"></a><a name="sequence-and-priority-in-planning"></a><a name="sequence-and-priority-in-planning"></a>Sequenz und Priorität in der Planung
...
### <a name="item-priority--low-level-code"></a><a name="item-priority--low-level-code"></a><a name="item-priority--low-level-code"></a>Element-Priorität / Low-Level Code
...
#### <a name="optimizing-performance-for-low-level-calculations"></a><a name="optimizing-performance-for-low-level-calculations"></a><a name="optimizing-performance-for-low-level-calculations"></a>Optimierung der Leistung für Low-Level-Berechnungen
...
### <a name="locations--transfer-level-priority"></a><a name="locations--transfer-level-priority"></a><a name="locations--transfer-level-priority"></a>Orte / Priorität auf Übertragungsebene
...
### <a name="order-priority"></a><a name="order-priority"></a><a name="order-priority"></a>Auftragspriorität
...
## <a name="demand-forecasts-and-blanket-orders"></a><a name="demand-forecasts-and-blanket-orders"></a><a name="demand-forecasts-and-blanket-orders"></a>Bedarfsplanung und Rahmenaufträge
...
## <a name="planning-assignment"></a><a name="planning-assignment"></a><a name="planning-assignment"></a>Planungszuordnung
...
## <a name="item-dimensions"></a><a name="item-dimensions"></a><a name="item-dimensions"></a>Dimensionen der Elemente
...
## <a name="item-attributes"></a><a name="item-attributes"></a><a name="item-attributes"></a>Artikelattribute
...
### <a name="specific-attributes"></a><a name="specific-attributes"></a><a name="specific-attributes"></a>Spezifische Attribute

Einige Nachfrageattribute sind spezifisch und ein Angebot muss ihnen genau entsprechen.

* Serien-/Chargennummern, die bestimmte Anwendung benötigen (Diese Attribute sind erforderlich, wenn Sie den Schalter **SN-spezifisches Tracking** oder **Chargenspezifisches Tracking** auf der Seite **Artikeltrackingcodekarte** für das Artikeltracking einschalten, das im Artikel verwendet wird.)  
...
## <a name="order-to-order-links"></a><a name="order-to-order-links"></a><a name="order-to-order-links"></a>Order-to-Order-Verbindungen
...
## <a name="reservations"></a><a name="reservations"></a><a name="reservations"></a>Reservierungen
...
## <a name="warnings"></a><a name="warnings"></a><a name="warnings"></a>Warnungen
...
### <a name="emergency"></a><a name="emergency"></a><a name="emergency"></a>Notfall
...
### <a name="exception"></a><a name="exception"></a><a name="exception"></a>Ausnahme
...
### <a name="attention"></a><a name="attention"></a><a name="attention"></a>Achtung
...
* Der voraussichtliche Lagerbestand übersteigt das Überlauflevel am Fälligkeitsdatum. Weitere Informationen finden Sie unter [Unter der Überlaufmenge bleiben](design-details-handling-reordering-policies.md#stay-below-the-overflow-level).  
...
## <a name="error-logs"></a><a name="error-logs"></a><a name="error-logs"></a>Fehlerprotokolle
...
Wenn das Feld nicht aktiviert ist, wird die Stapelverarbeitung **Planung berechnen** fortgesetzt, bis sie abgeschlossen ist. Fehler unterbrechen die Stapelverarbeitung nicht. Bei Fehlern gibt eine Meldung an, wie viele Artikel betroffen waren. Die Seite **Planungsfehlerprotokoll** bietet weitere Informationen zu den Fehlern sowie zu den Verknüpfungen für die betroffenen Belege oder Einstellungen.  

:::image type="content" source="media/nav_app_supply_planning_1_error_log.png" alt-text="Fehlermeldungen im Planungsarbeitsblatt.":::

## <a name="planning-flexibility"></a><a name="planning-flexibility"></a><a name="planning-flexibility"></a>Flexibilität bei der Planung
...
## <a name="order-planning"></a><a name="order-planning"></a><a name="order-planning"></a>Auftragsplanung
...
## <a name="finite-loading"></a><a name="finite-loading"></a><a name="finite-loading"></a>Begrenzte Ladung
...
Beim Planen mit eingeschränkter Kapazität stellt [!INCLUDE [prod_short](includes/prod_short.md)] sicher, dass Ressourcen nicht oberhalb ihrer definierten Kapazität (Grenzbelastung) geladen werden. Es weist jeden Arbeitsgang dem nächsten verfügbaren Zeitfenster zu. Wenn das Zeitfenster nicht ausreicht, um den Arbeitsgang abzuschliessen, wird der Arbeitsgang in mindestens zwei Teile im nächsten verfügbaren Zeitfenster aufgeteilt.  
