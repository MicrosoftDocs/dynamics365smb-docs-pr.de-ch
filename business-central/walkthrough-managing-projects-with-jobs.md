...
description: 'Diese exemplarische Vorgehensweise führt Sie in die Funktionen der Projektverwaltung ein, mit denen Sie die Nutzung der Ressourcen Ihrer Firma planen können.'
author: brentholtorf
ms.topic: conceptual
ms.devlang: al
...
Diese exemplarische Vorgehensweise umfasst die Einrichtung eines neuen Projekts und die damit verbundenen Aufgaben:

- Festpreise verwenden
- Teilzahlungen durchführen
- Rechnungen aus Projekten buchen
- Projekte kopieren
...
- Projektaufgabenzeilen und Planungszeilen einrichten  
- Projektspezifische Preise für Artikel, Ressourcen und Fibukonten erstellen  
- Debitoren ein Projekt in Rechnung stellen  

### <a name="handling-fixed-prices"></a>Festpreise verwenden

 Sie können Festpreise und die vorab mit Debitoren vereinbarten Preise für Services oder Waren verwendet werden. In dieser exemplarischen Vorgehensweise lernen Sie Folgendes:  

- Erfahren Sie, wie Vertrags- und Rechnungswerte ermittelt werden.  
- Zusätzliche, nicht fakturierte Arbeit einplanen  

### <a name="copying-a-project"></a>Ein Projekt kopieren

 In diesem Teil der exemplarischen Vorgehensweise wird gezeigt, wie Sie ein Projekt ganz oder teilweise kopieren, um den manuellen Dateneingabeaufwand zu reduzieren und die Genauigkeit zu verbessern.

- Einen Teil eines Projekts in ein neues Projekt kopieren  
- Projektspezifische Preise kopieren  
...
- Teilzahlungen für ein Projekt erstellen  
- Teilzahlungen fakturieren  
- Den Verbrauch in einem Projekt abrechnen, das für Teilzahlungen eingerichtet ist  
...
## <a name="prerequisites"></a>Voraussetzungen
 ...
 Bevor Sie die Aufgaben in dieser Demonstration ausführen, müssen Sie die folgenden Schritte ausführen:  
...
Im Mittelpunkt dieser exemplarischen Vorgehensweise steht CRONUS, ein fiktives Design- und Beratungsunternehmen, das neue Infrastrukturen entwirft und einrichtet. Zum Beispiel Konferenzräume und Büros mit Möbeln, Zubehör und Speichereinheiten. Der Grossteil der Arbeiten ist projektorientierter Natur. Prakash, ein Projektmanager bei CRONUS verwendet Projekte, um sich einen Überblick über die einzelnen laufenden Projekte zu verschaffen, die CRONUS gestartet und abgeschlossen hat. In der Regel ist es Prakash, der die Geschäfte mit Debitoren einrichtet und die Kerndaten des Projekts, d.h. Aufgaben- und Planungszeilen sowie Preise, in [!INCLUDE[prod_short](includes/prod_short.md)] erfasst. Prakash findet, dass erstellen, verwalten, und Informationen prüfen einfach ist. Auch Prakash schätzt die Art, wie [!INCLUDE[prod_short](includes/prod_short.md)] das Kopieren von Projekten sowie von Teilzahlungen ausführt.

 Katrin, ein Projektteammitglied, das Prakash unterstellt ist, ist für die tägliche Überwachung des Projekts zuständig. Tricia gibt die erledigte Arbeit der Techniker zu jeder Aufgabe ein und erfasst die verwendeten Artikel sowie die entstandenen Kosten.  

## <a name="preparing-sample-data"></a>Beispieldaten vorbereiten

Um sich für diese exemplarische Vorgehensweise vorzubereiten, müssen Sie Katrin als Ressource hinzufügen.  
...
Im folgenden Verfahren erstellen Sie einen Projekterfassungsjournal-Batch für Katrin, um deren Verbrauch zu buchen.  

### <a name="to-create-a-project-journal-batch"></a>So erstellen Sie einen Projekterfassungsjournal-Batch

1. Wählen Sie die ![Glühbirne, die die „Wie möchten Sie weiter verfahren“-Funktion öffnet.](media/ui-search/search_small.png "Wie möchten Sie weiter verfahren?") Symbol. Geben Sie **Projekterfassungsjournale** ein und wählen Sie dann den zugehörigen Link aus.  
2. Wählen Sie auf der Seite **Projekterfassungsjournal** das Feld **Batchname** aus. Die Seite **Projekterfassungsjournal-Batch** wird geöffnet.  
...
6. Stellen Sie auf dem Inforegister **Aussenhandel** sicher, dass der Währungscode auf **USD** festgelegt ist. Wenn Sie im Feld **Währungscode Rechnung** die Option „USD“ auswählen, wird das Projekt in US-Dollar fakturiert und nur in der lokalen Währung von CRONUS geplant.  

 Sie können die Preise für Debitoren auf Pro-Projekt-Basis je nach Ihren Vereinbarungen anpassen. Im folgenden Verfahren gibt der Projekt-Manager Kosten für Katrins Zeit an, setzt den Preis für die benötigte Software fest und fügt in den Reisekosten hinzu, dass der Debitor zugestimmt hat, zu zahlen.  
...
9. Schliessen Sie die Seite.  
 ...
 Die letzten Schritte beim Einrichten eines Projekts bestehen darin, die Projektaufgaben und die Planungszeilen hinzufügen, die Teil jeder Aufgabe sind. Die Planungszeilen bestimmen, welche Posten dem Kunden in Rechnung gestellt werden.  
...
2. Erstellen Sie Planungszeilen mit den folgenden Informationen:  
    ...
    | Zeile | Zeilenart | Planungsdatum  | Art        | Anz.   | Menge | VK-Preis |
    |------|-----------|----------------|-------------|-------|----------|------------|
...
3. Wählen Sie die Zeile 1020 und dann die Aktion **Projektplanzeilen** aus. Geben Sie die folgenden Informationen ein:  
    ...
    | Auftrag | Zeilenart | Planungsdatum  | Art        | Anz.   | Menge | VK-Preis |
    |------|-----------|----------------|-------------|-------|----------|------------|
...
## <a name="calculating-remaining-usage"></a>Restverbrauch berechnen

Katrin, das Teamprojektmitglied, arbeitet seit einiger Zeit an dem Projekt und möchte ihre Stunden und ihren Verbrauch für das Projekt erfassen. Katrin hat nicht mehr gearbeitet, als vorab mit dem Debitoren vereinbart wurde. Katrin verwendet den Batchauftrag **Restverbrauch berechnen**, um den Restverbrauch in einem Projekterfassungsjournal zu berechnen. Mithilfe des Batchauftrags wird für jede Projektaufgabe die Differenz zwischen dem geplanten Verbrauch von Artikeln, Ressourcen und Aufwandsfibuposten und dem in Projektposten gebuchten tatsächlichen Verbrauch berechnet. Der Restverbrauch wird dann im Projekterfassungsjournal angezeigt und Tricia kann eine Buchung vornehmen.  
...
2. Öffnen Sie auf der Seite **Projekt-Erf.-Journal** im Feld **Batchname** die Liste **Projekt-Erf.-Journal-Batches**. Wählen Sie den Projekterfassungsjournal-Namen **Katrin** aus.  
3. Wächlen Sie im Aktionsbereich **Restverbrauch**.  
4. Wählen Sie auf der Seite **Restverbrauch für Projekt berechnen** im Inforegister **Projektaufgabe** das **Projektnr.**- Feld aus und wählen Sie die Nummer des entsprechenden Projekts aus, in der Regel „Projekt J00010“.  
5. Geben Sie auf dem Inforegister **Optionen** die Nummer **PW00001** in das Feld **Belegnr.** ein. Durch diese Informationen wird das zukünftige Tracking der Buchung vereinfacht.  
6. Geben Sie als Buchungsdatum das heutige Datum ein.  
7. Wählen Sie die Schaltfläche **OK** aus. Diese Aktion generiert alle Projekterfassungsjournal-Zeilen basierend auf den von Prakash erstellten Planungszeilen.  
8. Wählen Sie die Schaltfläche **OK** aus auf der Bestätigungsseite. Die generierten Zeilen werden dem Projekterfassungsjournal hinzugefügt.  
...
## <a name="creating-and-posting-a-project-sales-invoice"></a>Eine Projektverkaufsrechnung erstellen und buchen

Als Nächstes kann Katrin eine neue Rechnung für das gesamte Projekt oder für einen Teil eines Projekts erstellen. Tricia kann die Rechnung auch an eine andere Rechnung für denselben Debitoren und dasselbe Projekt anfügen. In diesem Fall fakturiert Tricia das gesamte Projekt, da es jetzt abgeschlossen ist.  
...
Nachdem Tricia die Rechnung erstellt hat, ist sie beispielsweise im Rollencenter **Kundenauftragsabwickler** verfügbar.
...
1. Öffnen Sie das Projekt und dann die Aktion **Projektplanzeilen**.  
...
Nachdem Katrin dem Plan für diese Aufgabenzeile hinzugefügt wurde, arbeitet sie 25 Stunden an dem Projekt. Diese Stunden können im Projekterfassungsjournal erfasst werden.  

### <a name="to-enter-hours-in-a-project-journal"></a>So erfassen Sie Stunden in einem Projekterfassungsjournal
...
3. Wählen Sie die Aktion **Buchen** aus.  
     ...
     Ein paar Tage später arbeitet Tricia weitere 10 Stunden im Job und hat insgesamt 35 Stunden gearbeitet. Da Sie nur 30 Stunden mit dem Debitoren vereinbart haben, werden dem Debitoren nur fünf dieser Stunden in Rechnung gestellt. Tricia fügt dem Zeitplan die anderen fünf Stunden hinzu.  

4. Wählen Sie auf der Seite **Projekt-Erf.-Journal** die Option **Restverbrauch berechnen** aus.  
...
8. Erstellen Sie im **Projekt-Erf.-Journal** eine neue Zeile mit den folgenden Informationen. Prüfen Sie, ob beide Projektnummern Folgenummern der Nummern sind, die Sie bereits verwendet haben:  
    ...
    - **Menge**: **5**  
     ...
     Wenn Sie die Zeilenart **Budget** verwenden, werden die geplanten Kosten und der Preis aktualisiert, ohne die dem Debitoren in Rechnung gestellten Einstandsbeträge und Preise aus dem Vertrag zu aktualisieren.  
...
11. Wählen Sie das Projekt GUILDFORD und dann im Abschnitt **Projektaufgabenzeilen** Sie Zeile 1120 aus, und klicken Sie im Feld **Budget (Gesamtkosten)** mit der rechten Maustaste auf den Betrag. Wählen Sie **DrillDown** aus, um die Informationen anzuzeigen.  
     ...
     Sie können sehen, dass die Änderungen automatisch in der Zeile für die Projektaufgabennummer 1120 eingegeben wurden. Im Einstandsbetrag der geplanten Arbeit wurden dem Plan fünf weitere Arbeitsstunden von Katrin hinzugefügt.  
...
Im Einstandsbetrag des Vertrags werden nur die ursprünglich im Vertrag festgesetzten 30 Stunden angezeigt, wie mit dem Debitoren vereinbart.  
...
Auf der Seite **Projekt kopieren** können Sie das zu kopierende Projekt und die gewünschten Aufgabenzeilen auswählen.

- Kopieren Sie die Quellprojektposten, um Planungszeilen basierend auf der tatsächlichen Nutzung zu erstellen.
- Kopieren Sie die Planungszeilen des Quellprojekts, um die ursprünglichen Planungszeilen in das neue Projekt zu kopieren.

Danach können Sie die für das neue Projekt relevanten Planungszeilen- oder Projektpostenarten auswählen und in den Kopiervorgang einschliessen. Zum Schluss wählen Sie das Zielprojekt aus, und Sie legen fest, ob Preise und Mengen ebenfalls kopiert werden sollen.  

### <a name="to-copy-a-project"></a>So kopieren Sie ein Projekt

1. Wählen Sie die ![Glühbirne, die die „Wie möchten Sie weiter verfahren“-Funktion öffnet.](media/ui-search/search_small.png "Tell Me-Funktion") und geben Sie **Projekte** ein. Wählen Sie dann den zugehörigen Link aus.  
2. Wählen Sie die **Neu** Aktion aus, um ein neues Projekt zu erstellen. Geben Sie die folgenden Informationen ein:  
...
## <a name="making-payments-by-installments"></a>Teilzahlungen anwenden
...
2. Geben Sie auf der neuen Karte **Projekt** die folgenden Informationen ein:  
    ...
    - **Beschreibung**: **Neugestaltung der Empfangsregion**  
...
6. Erstellen Sie eine Planungszeile mit den folgenden Informationen:  
    ...
    | Zeile | Zeilenart | Planungsdatum  | Art        | Anz.   | Menge | VK-Preis |
    |------|-----------|----------------|-------------|-------|----------|------------|
    | 1    | Fakturierbar  | (heutiges Datum) | Fibukonto | 40255 | 0        | 5000       |
...
8. Erstellen Sie eine Planungszeile mit den folgenden Informationen:
    ...
    | Zeile | Zeilenart | Planungsdatum  | Art     | Anz.    | Menge |
    |------|-----------|----------------|----------|--------|----------|
