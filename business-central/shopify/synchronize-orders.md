...
# <a name="synchronize-and-fulfill-sales-orders" />Verkaufsaufträge synchronisieren und erfüllen
...
## <a name="set-the-import-of-orders-on-the-shopify-shop-card" />Legen Sie den Import von Bestellungen auf der Shopify Shop-Karte fest.
...
### <a name="shipment-method-mapping" />Zuordnung von Versandmethoden
...
### <a name="location-mapping" />Zuordnung von Standorten
...
## <a name="run-the-order-synchronization" />Führen Sie die Auftragssynchronisation aus
...
> [!NOTE]  
> Beim Filtern nach Tag sollten Sie die Filtertoken `@` und `*` verwenden. Wenn Sie beispielsweise Bestellungen importieren möchten, die *tag1* enthalten, verwenden Sie `@*tag1*`. `@` stellt sicher, dass das Ergebnis die Gross- und Kleinschreibung berücksichtigt, während `*` Aufträge mit mehreren Tags findet.
...
## <a name="review-imported-orders" />Importierte Bestellungen überprüfen
...
## <a name="create-sales-documents-in-business-central" />Verkaufsbelege in Business Central erstellen
...
### <a name="to-create-sales-documents" />So erstellen Sie Verkaufsbelege
...
### <a name="manage-missing-customers" />Fehlende Debitoren verwalten
...
* Sie können die **Auftraggeber Nr.** zuweisen. und **Rechnung an Debitor Nr.** direkt auf der Seite **Shopify-Bestellungen** zuweisen, indem Sie einen Debitor aus der Liste der vorhandenen Debitoren auswählen.
...
### <a name="how-the-connector-chooses-which-customer-to-use" />Wie der Konnektor auswählt, welcher Kunde verwendet werden soll
...
### <a name="different-processing-rules-for-orders" />Unterschiedliche Bearbeitungsregeln für Aufträge
...
8. Erstellen Sie einen wiederkehrenden Auftragswarteschlangeneintrag für Bericht 30104 **Aufträge von Shopify synchronisieren**. Wählen Sie **STORE** im Feld **Shopify Shop-Code** aus und verwenden Sie Filter, um alle Aufträge ausser denen zu erfassen, die der POS-Vertriebskanal erstellt. Zum Beispiel: **<>Verkaufsstelle**
...
### <a name="impact-of-order-editing" />Auswirkung der Bearbeitung von Bestellungen
...
## <a name="synchronize-shipments-to-shopify" />Lieferungen mit Shopify synchronisieren
...
### <a name="shipping-agents-and-tracking-url" />Zusteller und Tracking-URL
...
## <a name="returns-and-refunds" />Rückgaben und Rückerstattungen
...
>[!Note]
>Der unter **Shopify Shop-Karte** festgelegte Rückgabelagerort, einschliesslich leerer Lagerorte, wird auf der erstellten Gutschrift verwendet. Das System ignoriert die ursprünglichen Lagerorte aus Aufträgen oder Lieferungen.

## <a name="gift-cards" />Geschenkkarten
