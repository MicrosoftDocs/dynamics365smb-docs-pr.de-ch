...
# <a name="troubleshooting-the-shopify-and-business-central-synchronization" />Fehlerbehebung bei der Synchronisierung zwischen Shopify und Business Central
...
## <a name="run-tasks-in-the-foreground" />Aufgaben im Vordergrund ausführen

1. Wählen Sie die ![Glühbirne, die die „Wie möchten Sie weiter verfahren“-Funktion öffnet 1.](../media/ui-search/search_small.png "Wie möchten Sie weiter verfahren?") , geben Sie **Shopify-Shop** ein, und wählen Sie den entsprechenden Link aus.
...
### <a name="to-enable-logging" />Um die Protokollierung zu aktivieren

1. Wählen Sie die ![Glühbirne, die die „Wie möchten Sie weiter verfahren“-Funktion 1.](../media/ui-search/search_small.png "Wie möchten Sie weiter verfahren?") öffnet. , geben Sie **Shopify-Shop** ein, und wählen Sie den entsprechenden Link aus.
2. Wählen Sie den Shop aus, für den Sie Probleme lösen möchten, um die Seite **Shopify-Shop-Karte** zu öffnen.
3. Schalten Sie den Schalter **Log Enabled** ein.

### <a name="to-review-logs" />Um die Protokolle zu überprfen
...
Denken Sie daran, die Protokollierung später auszuschalten, um negative Auswirkungen auf die Leistung und eine Vergrösserung der Datenbank zu vermeiden.
...
## <a name="data-capture" />Datenerfassung
...
## <a name="reset-sync" />Synchronisierung zurücksetzen
...
## <a name="request-the-access-token" />Fordern Sie das Access-Token an
...
## <a name="verify-and-enable-permissions-to-make-http-requests-in-a-non-production-environment" />Überprüfen und aktivieren Sie die Berechtigungen für HTTP-Anfragen in einer nicht produktiven Umgebung
...
1. Wählen Sie die ![Glühbirne, die die „Wie möchten Sie weiter verfahren“-Funktion 1.](../media/ui-search/search_small.png "Tell Me-Funktion") öffnet. Symbol, geben Sie **Erweiterungsverwaltung** ein und wählen Sie dann den zugehörigen Link.
...
## <a name="rotate-the-shopify-access-token" />Shopify-Zugriffstoken rotieren
...
### <a name="in-includeprodshortincludesprodshortmd" />In [!INCLUDE[prod_short](../includes/prod_short.md)]
...
## <a name="known-issues" />Bekannte Probleme

### <a name="error-the-sales-header-does-not-exist-identification-fields-and-values-document-typequotenoyour-shopify-store" />Fehler: Der Verkaufskopf existiert nicht. Identifizierungsfelder und -werte: Dokumententyp='Quote',No.='YOUR SHOPIFY STORE'
...
### <a name="error-gen-bus-posting-group-must-have-a-value-in-customer-your-shopify-store-it-cannot-be-zero-or-empty" />Fehler: Gen. Die Geschäftsbuchungsgruppe muss einen Wert in Debitor haben: „IHR SHOPIFY-GESCHÄFT“. Sie darf nicht leer sein
...
### <a name="error-importing-data-to-your-shopify-shop-isnt-enabled-go-to-the-shop-card-to-enable-it" />Fehler: Importieren von Daten in Ihren Shopify Shop ist nicht aktiviert. Gehen Sie zur Shop-Karte, um sie zu aktivieren
...
### <a name="error-oauth-error-invalidrequest-could-not-find-shopify-api-application-with-apikey" />Fehler: Oauth Fehler invalid_request: Konnte Shopify API-Anwendung mit api_key nicht finden
...
### <a name="error-internal-error-looks-like-something-went-wrong-on-our-end-request-id-xxxxxxxx-xxxx-xxxx-xxxx-xxxx" />Fehler: Interner Fehler. Anscheinend ist auf Ihrer Seite ein Fehler aufgetreten. Anforderungs-ID: XXXXXXXX-XXXX-XXXX-XXXX-XXXX
