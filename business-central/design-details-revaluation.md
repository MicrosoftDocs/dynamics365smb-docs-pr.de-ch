...
ms.author: edupont
---
# <a name="design-details-revaluation" />Designdetails: Neubewertung
...
## <a name="calculating-the-revaluable-quantity" />Berechnung der neubewertbaren Menge
 Die neu bewertbare Menge ist die Restmenge im Bestand, die für die Neubewertung an einem vorgegebenen Datum verfügbar ist. Sie wird als die Gesamtsumme der Mengen vollständig fakturierter Lagerposten berechnet, die ein Buchungsdatum gleich oder vor dem Neubewertungsbuchungsdatum haben.  

> [!NOTE]  
>  Artikel mit der Lagerabgangsmethode "Standard" werden bei der Berechnung der neubewertbaren Menge pro Artikel, Menge, Lagerort und Variante unterschiedlich berechnet. Die Mengen und Werte von Lagerposten, die noch nicht vollständig fakturiert wurden, werden in der neu bewertbaren Menge eingeschlossen.  
...
**1Q**: Der Benutzer bucht die eingekauften Links als erhalten. Die folgende Tabelle zeigt den sich daraus ergebenden Lagerposten.  

|Buchungsdatum|Artikel|Postentyp|Menge|Laufnr.|  
|------------------|----------|----------------|--------------|---------------|  
...
|Buchungsdatum|Postentyp|Bewertungsdatum|Einstandsbetrag (tatsächl.)|Lagerposten Laufnr.|Laufnr.|  
|------------------|----------------|--------------------|----------------------------|---------------------------|---------------|  
|01-15-20|EK-Preis|01-01-20|150.00|1|1|  

 **2Q + 2V**: Der Benutzer bucht die eingekauften Links als verbraucht für die Produktion der Eisenkette. Aus finanziellen Gesichtspunkten werden die Links Teil des WIP-Bestands.  Die folgende Tabelle zeigt den sich daraus ergebenden Lagerposten.  
...
## <a name="expected-cost-in-revaluation" />Soll-Kosten in der Neubewertung
Die neu bewertbare Menge wird als die Gesamtsumme der Mengen vollständig fakturierter Lagerposten berechnet, die ein Buchungsdatum gleich oder vor dem Neubewertungsbuchungsdatum haben. Das bedeutet, dass, wenn mehrere Artikel eingegangen/geliefert, aber noch nicht fakturiert sind, deren Lagerwert nicht berechnet werden kann. Artikel mit der Lagerabgangsmethode Standard werden nicht in dieser Hinsicht begrenzt.  
...
Wenn Sie die neu bewertbare Menge für Artikel mit der Lagerabgangsmethode Standard wird, werden Lagerposten, die noch nicht vollständig fakturiert wurden, in die Berechnung einbezogen. Diese Posten werden dann neu bewertet, wenn Sie die Neubewertung buchen. Wenn Sie den neu bewerteten Posten fakturieren, werden die folgenden Wertposten erzeugt:  
...
|Schritt|Buchungsdatum|Postentyp|Bewertungsdatum|Einstandsbetrag (erwartet)|Einstandsbetrag (tatsächl.)|Lagerposten Laufnr.|Laufnr.|  
|----------|------------------|----------------|--------------------|------------------------------|----------------------------|---------------------------|---------------|  
...
## <a name="determining-whether-an-inventory-decrease-is-affected-by-revaluation" />Bestimmen Sie, ob eine Bestandsreduzierung von der Neubewertung betroffen ist
...
|Szenario|Laufnr.|Zeitablauf|Beeinflusst durch Neubewertung|  
|--------------|---------------|------------|-----------------------------|  
...
|Szenario|Buchungsdatum|Postentyp|Bewertungsdatum|Bewertete Menge|Einstandsbetrag (tatsächl.)|Lagerposten Laufnr.|Laufnr.|  
|--------------|------------------|----------------|--------------------|---------------------|----------------------------|---------------------------|---------------|  
...
## <a name="wip-inventory-revaluation" />"Lagerbewertung - Aktiviert"
...
Das Produktionslager kann neubewertet werden, solange das Neubewertungsdatum nicht hinter dem Buchungsdatum des entsprechenden Lagerposten des Typs Verbrauch liegt und solange der entsprechende Fertigungsauftrag noch nicht fakturiert wurde.  
